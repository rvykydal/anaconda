...
class NetworkModule(KickstartModule):
    ...
    def install_network_with_task(self, sysroot, onboot_ifaces, overwrite):
        ...
        if self._should_apply_onboot_policy() and \
                not self._has_any_onboot_yes_device(self._device_configurations or []):
    ...
    def _should_apply_onboot_policy(self):
        ...
        Not if (any) devices were configured by kickstart or in UI.
        """
        return not self._original_network_data and not self._use_device_configurations

    def _has_any_onboot_yes_device(self, device_configurations):
        """Does any device have ONBOOT value set to 'yes'?"""
        uuids = [dev_cfg.uuid for dev_cfg in device_configurations
                 if dev_cfg.uuid]
        for uuid in uuids:
