...
from pyanaconda.modules.network.ifcfg import get_ifcfg_file_of_device, update_onboot_value, \
    update_slaves_onboot_value, get_dracut_arguments_from_ifcfg
...
class NetworkModule(KickstartModule):
    ...
    def dump_missing_ifcfg_files(self):
        ...
        for device in self.nm_client.get_devices():
            ...
            if n_cons > 1:
                ...
                device_is_slave = ac and ac.get_master()
                # For slaves multiple connections are expected
                if not device_is_slave:
                    log.debug("dump missing ifcfgs: %d connections found for device %s",
                              n_cons, iface)
                continue
            if n_cons == 1:
                con = cons[0]
    ...
    def get_dracut_arguments(self, iface, target_ip, hostname):
        ...
        if iface not in (device.get_iface() for device in nm_client.get_devices()):
