...
def create_connections_from_ksdata(nm_client, network_data, device_name, ifname_option_values=None):
    ...
    # type "bridge"
        ...
        for i, slave in enumerate(network_data.bridgeslaves.split(","), 1):
            slave_con = create_slave_connection('bridge', i, slave, device_name, network_data.onboot)
            bind_connection(nm_client, slave_con, network_data.bindto, slave)
            connections.append((slave_con, slave))
...
def _update_bond_kickstart_network_data(nm_client, iface, connection, network_data):
    bond_slaves = get_slaves_from_connections(
        nm_client,
        ['bond'],
        [iface, connection.get_uuid()]
    )
    if bond_slaves:
        slave_ifaces = sorted(s_iface
                              for _name,
                              s_iface, _uuid in bond_slaves if s_iface)
    ...
    if s_bond:
        ...
        if option_list:
            network_data.bridgeopts = ",".join(option_list)
...
def _update_bridge_kickstart_network_data(nm_client, iface, connection, network_data):
    ...
    bridge_slaves = get_slaves_from_connections(
        nm_client,
        ['bridge'],
        [iface, connection.get_uuid()]
    )
    if bridge_slaves:
        slave_ifaces = sorted(s_iface
                              for _name,
                              s_iface, _uuid in bridge_slaves if s_iface)
...
def _update_team_kickstart_network_data(nm_client, iface, connection, network_data):
    team_slaves = get_slaves_from_connections(
        nm_client,
        ['team'],
        [iface, connection.get_uuid()]
    )
    if team_slaves:
        slave_list = sorted((s_iface, s_uuid)
                            for _name, s_iface, s_uuid in team_slaves
                            if s_iface)

    for s_iface, s_uuid in slave_list:
        team_port_cfg = get_team_port_config_from_connection(nm_client, s_uuid) or ""
        team_port_cfg = team_port_cfg.replace("\n", "")
