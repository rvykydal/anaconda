...
import socket
from pyanaconda.modules.network.network_kickstart import default_ks_vlan_interface_name, \
    BIND_TO_MAC
...
def add_connection_from_ksdata(network_data, device_name, activate=False):
    ...
    if network_data.bondslaves:
        ...
        for i, slave in enumerate(network_data.bondslaves.split(","), 1):
            ...
            bind_connection(con, network_data.bindto, slave)
...
def ensure_active_connection_for_device(self, uuid, device_name, only_replace=False):
    ...
    if device:
        ...
        if ac or not only_replace:
            ...
            if uuid != active_uuid:
                ifcfg_con = self.nm_client.get_connection_by_uuid(uuid)
                # TODO sync somewhere?
