...
class ConfigureWirelessNetworksDialog(GUIObject):
    builderObjects = ["configure_wireless_network_dialog", "liststore_configure_wireless_network"]
    ...
    def initialize(self):
        ...
        selection.set_mode(Gtk.SelectionMode.BROWSE)
        self._connect_button = self.builder.get_object("wireless_configure_button")
        selection.connect("changed", self.on_selection_changed)
        self.on_selection_changed(selection)

    def on_selection_changed(self, selection):
        model, itr = selection.get_selected()
        self._connect_button.set_sensitive(itr)
...
class NetworkSpoke(FirstbootSpokeMixIn, NormalSpoke):
    """
       .. inheritance-diagram:: NetworkSpoke
          :parts: 3
    """
    builderObjects = ["networkWindow", "liststore_devices", "add_device_dialog", "liststore_add_device"]
...
class NetworkStandaloneSpoke(StandaloneSpoke):
    """
       .. inheritance-diagram:: NetworkStandaloneSpoke
          :parts: 3
    """
    builderObjects = ["networkStandaloneWindow", "networkControlBox_vbox", "liststore_devices", "add_device_dialog", "liststore_add_device"]
